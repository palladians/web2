...
const DynamicPage = async ({ params }: { params: { slug: string } }) => {
  ...
  return (
    <div className="container max-w-[56rem] flex flex-col gap-8 py-16">
      <h1 className="text-5xl font-semibold leading-[4rem]">{page.name}</h1>
      <ReactMarkdown className="prose lg:prose-lg dark:prose-invert max-w-none">{page.content}</ReactMarkdown>
    </div>
  )
}
